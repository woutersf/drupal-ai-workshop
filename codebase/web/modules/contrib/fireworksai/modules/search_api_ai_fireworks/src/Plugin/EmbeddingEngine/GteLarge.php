 ...
 */
#[EmbeddingEngine(
  id: 'fireworks_gte_large',
  label: new TranslatableMarkup('Fireworks GTE Large'),
  description: new TranslatableMarkup('Fireworks GTE Large.'),
  dimension: 1024,
)]
final class GteLarge implements EmbeddingEngineInterface {
  ...
  public function generateEmbeddings(string $text, array $options = []): array {
    ...
    $response = json_decode($this->client->embeddingsCreate('thenlper/gte-large', $text, $options), TRUE);
